...
function fmtPhp(amount: number | undefined) {
  return new Intl.NumberFormat("en-US", {
    style: "currency",
    currency: "PHP",
  }).format(amount ?? 0);
...
const PDFDownloadButton = dynamic(
  () => import("@react-pdf/renderer").then((mod) => mod.PDFDownloadLink),
  {
    ssr: false,
    loading: () => (
      <Button disabled size="sm" className="flex items-center gap-2 h-9">
...
export default function StaffInvoiceDetailPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8 pb-12 pt-4 px-4 sm:px-6 lg:px-8">
      {/* Top Bar */}
      <div className="flex items-center justify-between">
        ...
        <div className="hidden sm:flex items-center gap-3">
          ...
          >
            {/* @ts-ignore - The react-pdf render props types are notoriously loose */}
            {({ loading }) => (
              <Button
                size="sm"
                disabled={loading}
      ...
      {/* Timestamps & Notes (moved outside to sit quietly below) */}
          ...
          {invoice.eodReports && invoice.eodReports.length > 0 ? (
                ...
                <TableBody>
                  {invoice.eodReports.map((eod) => (
                    ...
                    >
                      ...
                      <TableCell className="max-w-[400px] truncate text-sm text-muted-foreground group-hover:text-foreground transition-colors">
                        {eod.tasksCompleted || "—"}
                      </TableCell>
                    </TableRow>
                  ))}
            ...
            <div className="flex flex-col items-center justify-center gap-3 py-16 text-center px-4">
              ...
              <p className="text-sm text-muted-foreground max-w-sm">
                No detailed EOD reports were linked to this invoice at the time
                of calculation.
              </p>
            </div>
          )}
