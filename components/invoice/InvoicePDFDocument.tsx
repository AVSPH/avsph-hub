...
export function InvoicePDFDocument({ invoice }: { invoice: any }) {
    ...
    const staffFullName =
        invoice.staffName ||
        (invoice.staff ? `${invoice.staff.firstName} ${invoice.staff.lastName}` : "Unknown Staff");
    const staffEmail = invoice.staffEmail || invoice.staff?.email || "—";
    const staffPosition = invoice.staffPosition || invoice.staff?.position || "—";

    const manualDeductions = invoice.deductions.filter((d: any) => !STATUTORY_TYPES.has(d.type));
    ...
    const totalManualDeductions = manualDeductions.reduce((sum: number, d: any) => sum + d.amount, 0);
    const totalAdditions = invoice.additions.reduce((sum: number, a: any) => sum + a.amount, 0);
            ...
            <Page size="A4" style={styles.page}>
                ...
                <View style={styles.headerRow}>
                    <View>
                        ...
                        <Text style={styles.title}>{fmtPeriod(invoice.periodStart, invoice.periodEnd)}</Text>
                    </View>
                ...
                <View style={styles.gridRow}>
                    ...
                    <View style={styles.col}>
                        ...
                        <View style={styles.flexRow}>
                            ...
                            <Text style={{ fontFamily: "Helvetica-Bold" }}>{invoice.totalHoursWorked.toFixed(1)} hrs</Text>
                        </View>
                        <View style={styles.flexRow}>
                            <Text style={styles.mutedText}>Days Worked</Text>
                            <Text style={{ fontFamily: "Helvetica-Bold" }}>{invoice.totalDaysWorked} days</Text>
                        </View>
                    </View>
                </View>
                ...
                <View style={styles.breakdownSection}>
                    ...
                    <View style={styles.row}>
                        <Text style={{ fontFamily: "Helvetica-Bold" }}>Regular Earnings</Text>
                    ...
                    <View style={styles.row}>
                        ...
                        <Text>{fmt(invoice.earningsBreakdown?.sundayPremiumEarnings || 0)}</Text>
                    </View>
                    <View style={styles.row}>
                        <Text style={styles.mutedText}>Night Differential</Text>
                        <Text>{fmt(invoice.earningsBreakdown?.nightDifferentialEarnings || 0)}</Text>
                    </View>
                    <View style={styles.row}>
                        <Text style={styles.mutedText}>Rice Allowance</Text>
                        <Text>{fmt(invoice.earningsBreakdown?.riceAllowanceEarnings || 0)}</Text>
                    </View>
                    ...
                    {invoice.additions.length > 0 && (
                        <View>
                            <Text style={[styles.sectionHeading, styles.addText]}>Additions</Text>
                    ...
                    {/* Statutory */}
                    <View>
                        <Text style={[styles.sectionHeading, styles.dedText]}>Statutory Deductions</Text>
                        <View style={styles.row}>
                            <Text style={styles.mutedText}>SSS</Text>
                            <Text style={styles.dedText}>-{fmt(invoice.statutoryDeductions?.sss || 0)}</Text>
                        </View>
                        <View style={styles.row}>
                            <Text style={styles.mutedText}>Pag-IBIG</Text>
                            <Text style={styles.dedText}>-{fmt(invoice.statutoryDeductions?.pagIbig || 0)}</Text>
                        </View>
                        <View style={styles.row}>
                            <Text style={styles.mutedText}>PhilHealth</Text>
                            <Text style={styles.dedText}>-{fmt(invoice.statutoryDeductions?.philHealth || 0)}</Text>
                        </View>
                        <View style={[styles.rowBold, styles.separator]}>
                            <Text>Total Statutory Deductions</Text>
                            <Text style={styles.dedText}>-{fmt(totalStatutoryDeductions)}</Text>
                        </View>
                    </View>
                    ...
                    {manualDeductions.length > 0 && (
                        <View>
                            <Text style={[styles.sectionHeading, styles.dedText]}>Other Deductions</Text>
                            ...
                            <View style={[styles.rowBold, styles.separator]}>
                                ...
                                <Text style={styles.dedText}>-{fmt(totalManualDeductions)}</Text>
                            </View>
                        </View>
                    )}
                </View>
                ...
                <View style={styles.footer}>
                    <View>
                        ...
                        <Text style={styles.footerDesc}>Calculated Pay + Additions - Deductions</Text>
                    </View>
